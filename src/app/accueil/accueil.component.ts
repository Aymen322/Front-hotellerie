...
filterStarNumber() : void{
  this.filteredStarsNumber = this.hotels.filter(hotel =>
    hotel.name.toLowerCase().includes(this.searchStarsNumber.toLowerCase())
  );
}
...
selectStarNumber(hotel: Hotel) {
  this.searchStarsNumber = hotel.starnumber;
  this.filteredStarsNumber = []; // Clear suggestions after selection
  ...
  search(): void {
    if (this.searchRegion.trim() !== '' || this.searchStarsNumber.trim() !== '') {
      ...
      if (this.searchStarsNumber.trim() !== '') {
        queryParams.starsNumber = this.searchStarsNumber;
      }
